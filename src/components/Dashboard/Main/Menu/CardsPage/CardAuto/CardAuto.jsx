...
import "../../../../../../Styles.css";
import AddSubCardModal from "./AddSubCardAuto/AddSubCardModalAuto";
...
//al doilea pas
const loadCardsFromFirestore = async (user, setCardsAuto) => {
  try {
    const docRef = doc(db, `users/${user.uid}/componentA`, "cardsAuto");
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      setCardsAuto(docSnap.data().cardsAuto || []); // Aici folosim cardsAuto
    } else {
      console.log("No cardsAuto found for the user.");
    }
  } catch (error) {
    console.error("Error loading cardsAuto: ", error);
  }
};

//primul pas
const saveCardsToFirestore = async (user, newCardsAuto) => {
  try {
    // Creează un document sub componentA cu numele cardsAuto
    const docRef = doc(db, `users/${user.uid}/componentA`, "cardsAuto");

    // Salvează datele sub câmpul cardsAuto, nu cards
    await setDoc(docRef, { cardsAuto: newCardsAuto }, { merge: true });

    console.log("CardsAuto saved successfully for ComponentA.");
  } catch (error) {
    console.error("Error saving cardsAuto for ComponentA: ", error);
  }
};
    ...
    <MDBContainer fluid className={styles.customBackground}>
      <MDBRow>
        <MDBCol className="text-center z-1">
          <h2 className="mb-3 custom-font">Create an Event for Auto</h2>
