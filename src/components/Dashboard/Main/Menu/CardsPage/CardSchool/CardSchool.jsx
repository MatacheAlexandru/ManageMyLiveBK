...
import "../../../../../../Styles.css";
import AddSubCardModal from "./AddSubCardSchool/AddSubCardModlSchool";
...
//al doilea pas
const loadCardsFromFirestore = async (user, setCardsSchool) => {
  try {
    const docRef = doc(db, `users/${user.uid}/componentS`, "cardsSchool"); // Folosește componentP și cardsPet
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      setCardsSchool(docSnap.data().cardsSchool || []); // Folosește cardsPet
    } else {
      console.log("No cardsSchool found for the user.");
    }
  } catch (error) {
    console.error("Error loading cardsSchool: ", error);
  }
};

//primul pas
const saveCardsToFirestore = async (user, newCardsSchool) => {
  try {
    const docRef = doc(db, `users/${user.uid}/componentS`, "cardsSchool"); // Folosește componentP și cardsPet
    await setDoc(docRef, { cardsSchool: newCardsSchool }, { merge: true });
    console.log("CardsSchool saved successfully for ComponentS.");
  } catch (error) {
    console.error("Error saving cardsSchool for ComponentS: ", error);
  }
};
    ...
    <MDBContainer fluid className={styles.customBackground}>
      <MDBRow>
        <MDBCol className="text-center z-1">
          <h2 className="mb-3 custom-font">Create an Event for School</h2>
