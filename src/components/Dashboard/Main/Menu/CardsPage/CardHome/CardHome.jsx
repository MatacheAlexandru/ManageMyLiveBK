...
//al doilea pas
const loadCardsFromFirestore = async (user, setCardsHome) => {
  try {
    const docRef = doc(db, `users/${user.uid}/componentH`, "cardsHome"); // Folosește componentP și cardsPet
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      setCardsHome(docSnap.data().cardsHome || []); // Folosește cardsPet
    } else {
      console.log("No cardsHome found for the user.");
    }
  } catch (error) {
    console.error("Error loading cardsHome: ", error);
  }
};

//primul pas
const saveCardsToFirestore = async (user, newCardsHome) => {
  try {
    const docRef = doc(db, `users/${user.uid}/componentH`, "cardsHome"); // Folosește componentP și cardsPet
    await setDoc(docRef, { cardsHome: newCardsHome }, { merge: true });
    console.log("CardsHome saved successfully for ComponentH.");
  } catch (error) {
    console.error("Error saving cardsHome for ComponentH: ", error);
  }
};
...
const CardItem = ({
  ...
  return (
    <MDBCol sm="12" className="mb-4 p-0">
